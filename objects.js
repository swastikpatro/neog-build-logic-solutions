...
//   const studentsWithTotal = [...structuredClone(studentsDataIncludingTotalMarks(studentsObj))];
...
//     const studentsWithTotalAndGrade = studentsWithTotal.map(({name, grade}) => {
...
//   const studentsWithTotal = [...structuredClone(studentsDataIncludingTotalMarks(studentsObj))];
//   const passedStudents = studentsWithTotal.filter(({total}) => isPass(total, 4));
//   const nameOfPassedStudentsArr = passedStudents.map(({name}) => name);
//   return `Total ${nameOfPassedStudentsArr.length} students passed namely: ${nameOfPassedStudentsArr.join(', ')}`;
// }
